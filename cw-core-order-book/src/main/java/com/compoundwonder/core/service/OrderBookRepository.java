 ...
 *
 * <p>{@link #put(int, OrderBook)} 和 {@link #clear()} 仅供引擎注册、重置等生命周期操作使用。
 * {@link #get(int)} 返回 Handler 正在维护的订单簿引用，普通调用方只能读取，
...
public interface OrderBookRepository {
     ...
     * 在行情进入前注册订单簿。
     */
    void put(int symbolId, OrderBook orderBook);

    /**
     * 获取共享订单簿引用。调用方不得修改返回对象。
     */
    OrderBook get(int symbolId);
