...
- `StockWatchingTaskServiceImpl`：入口、候选池、过滤编排、评分、排序和落库；
- `StockSelectionAssistDTO`：所有过滤与评分字段口径；
- `StockChipFilter`：200 根 K 线筹码指标和普通筹码阶梯；
...
flowchart TD
    ...
    C3 --> E[写入 stock_watching_task]
    D7 -->|否| E
    D8 --> E
```

入口先按选股日期查询当日存在有效可转债的正股代码。首板和连板候选都排除这些股票，然后分别生成两个交易模式的任务。
...
- 90 日历史最高板：筹码窗口中 `[本轮首板前一交易日 - 90个自然日, 本轮首板前一交易日]` 范围内的最高板。

### 3.5 形态指标

- 非正常 K 线：`klineState != 0`。
- 一字板计数：本轮连板日中满足 `klineState == 3` 或当日振幅 `< 1.5%` 的数量。
...
该分支在普通首板 Top3 之后执行，只处理尚未入选的首板基础候选。
...
它仍然受首板初始候选池约束，因此当日流通市值仍需 `<30亿元`、当日收盘价 `<40元`、当日涨跌幅 `<11%`。
...
满足条件的小市值候选先按任务统一排序规则排列，再最多追加 2 只。因此首板最终最多 5 只：普通严格首板 Top3，加小市值首板 Top2。

当历史筹码数据不足时，`200根K线历史最高板` 的空值按 `0` 处理。业务确认新股不考虑这种历史不足情况，当前不增加额外拒绝逻辑。
...
| 2 | 本轮一字板数量 `<2次` |
| 3 | 不满足“历史最大换手 `<=25%` 且非 ST 月份 `<18` 且非 ST 月份 `<` 上市月份” |
| 4 | 通过连板近期形态过滤 |
...
普通首板、正常严格连板和弱 5 板兜底使用完整筹码过滤。冰点通道只使用其中的历史硬限制；小市值首板不使用完整筹码过滤。
...
每种交易模式分别按 `recommendDate + tradeMode` 删除旧任务，再保存新任务。任务保存：股票代码、名称、分数、连板数、推荐日期、下一交易日、交易模式和创建时间。
