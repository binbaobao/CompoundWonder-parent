...
//        }
//        // 初始化持仓数据盯盘，同步盯盘状态
//        realTradeService.initStockAccountInfo(date);
...
//        // 添加行情数据（逐笔 + 3秒tick）订阅
//        marketStreamService.xmdApiInit();
//        marketStreamService.level2ApiInit();
//        // 查询交易系统的涨跌停价
//        for (String orderBookCode : cacheService.getOrderBookCodes()) {
//            realTradeService.queryOrderBookInfo(orderBookCode);
//            Thread.sleep(1000);
//        }
//    }
//
...
//                // 如果有擒龙捉妖的票已经下单了就终止方法
//                List<TstpOrderDto> orderDtos = realTradeService.findOrderBySecurityID(qinLongCode);
//                try {
...
//                List<TstpOrderDto> entrustmentRecords = realTradeService.findEntrustmentRecords(orderBook.getSymbol());
...
//        tickEventShangHaiHandler = new TickEventShangHaiHandler( realTradeService);
//        tickEventShenZhenHandler = new TickEventShenZhenHandler( realTradeService);
